...
class AttentionLayer(nn.Module):
    ...
    def forward(self, w, r, w_bias, r_bias, theme_var_encoding, theme_var_bias):
        r = self.r_net(r)
        q = self.q_net(w)
        k = self.k_net(w)
        v = self.v_net(w)
        ...
        k_len = k.size(1)
        r_len = r.size(1)
        tv_len = theme_var_encoding.size(1)
        ...
        tv = theme_var_encoding.expand([self.n_head, -1, -1, -1]).transpose(0, 1).contiguous()
        ...
        theme_var_bias_expend = theme_var_bias.expand([batch_size, -1, -1])
        tv = tv.view(batch_size, self.n_head, -1)
        tv_pos = theme_var_bias_expend * tv
        tv_pos = tv_pos.view(batch_size, self.n_head, tv_len, tv_len)
        # import pdb; pdb.set_trace()

        attn_score_org = (ac + bd) * self.scale
        attn_score_tv = (attn_score_org.mul(tv_pos)) * self.scale

        attn_score = attn_score_org + attn_score_tv
